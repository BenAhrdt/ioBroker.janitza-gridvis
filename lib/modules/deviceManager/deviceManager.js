...
class GridVisDeviceManagement extends DeviceManagement {
    ...
    async getInstanceInfo() {
        ...
        info.actions.push({
            id: 'GridVisVersion',
            title: `GridVis Server Version: ${(await this.adapter.getStateAsync(`${this.adapter.namespace}.info.gridVisVersion`)).val}`,
            icon: 'lines',
            description: 'GridVis-Version',
            //url: `http://${this.adapter.config.address}:${this.adapter.config.port}`,
        });
        return info;
    }
        ...
        for (const [deviceId, deviceValue] of Object.entries(this.adapter.objectStore.devices)) {
            ...
            for (const [key, value] of Object.entries(deviceValue.onlineValues || {})) {
                ...
                for (const [stateKey, stateValue] of Object.entries(value)) {
                    ...
                    let card = {
                        preLabel: `${this.replaceNameing(value.object.common.name)} `,
                        name: ` ${stateValue.object.common.name}`,
                    };
            ...
            const sortedItems = Object.fromEntries(
                Object.entries(res.customInfo.schema.items).sort(([, a], [, b]) => {
    ...
    replaceNameing(name) {
        switch (name) {
            case 'Bezogene Wirkarbeit':
                return 'Bezogen';

            case 'Gelieferte Wirkarbeit':
                return 'Geliefert';
    ...
    async getDeviceDetails(id) {
        ...
        if (this.adapter.objectStore.devices[id].info) {
            deviceInfoItems[`Header`] = {
                newLine: true,
                type: 'header',
                text: `deviceInfo`,
                size: 3,
            };
            const deviceInfo = this.adapter.objectStore.devices[id].info;
            const sortedEntries = Object.entries(deviceInfo)
                .filter(([k]) => k !== 'object')
                .sort(([, a], [, b]) =>
                    a.object.common.name.localeCompare(b.object.common.name, { sensitivity: 'base' }),
                );
            for (const [k, v] of sortedEntries) {
                ...
                const label = deviceInfo[k].object.common.name;
                deviceInfoItems[`${label}`] = {
                    newLine: true,
                    type: 'state',
                    label: label,
                    oid: v.object._id,
